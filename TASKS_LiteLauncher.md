...
| 完成 | 编号 | 任务 | 优先级 | 状态 | 备注 |
|---|---|---|---|---|---|
| [x] | LL-207 | Cashflow 复盘功能实装 | P1 | 完成 | `cash review` 读取最近 12 局持久化对局，可切换活动/归档记录，并展示综合建议、现金/被动收入/净资产指标与结构化决策时间线；刷新不会重复打开面板，结束对局也不会在复盘入口自动重开 |
| [x] | LL-208 | Cashflow AI 多性格策略 | P1 | 完成 | 内置稳健、均衡、进取三种离线策略，分别使用不同风险、贷款、现金储备和 ROI 权重；同机会可产生可解释的不同决策，旧单 AI 存档会自动补齐且不重复，主面板与复盘均展示性格说明 |
...
| [x] | LL-404 | 自动更新端到端验证 | P2 | 完成 | 已完成 `v1.0.24 -> v1.0.25` 真实链路验证；新增 `verify:update:live` 只读校验 GitHub Latest、`latest.yml`、NSIS、blockmap、大小和发布说明，可验证 `v1.1.15 -> v1.1.16` 而不下载安装包；NSIS 安装/升级仍只允许一次性 Windows 环境 |
...
| [ ] | LL-505 | E2E 自动化（搜索 + 插件 + 设置） | P2 | 进行中 | 已覆盖搜索首页布局、Windows Codex、WebTools、设置诊断、窗口失焦、125%/150% 缩放；新增 `test:e2e:litesnap:real`，使用真实 Windows 原生区域截图和滚轮完成向下、向上、再到底部，校验双端标记、方向切换及虚线框排除；继续按真实缺陷补充 |
...
| [x] | LL-508 | 发布后兼容与稳定性门禁 | P1 | 完成 | Cashflow 覆盖旧单 AI 多次水合/持久化去重、14 局频繁归档、最近 12 局复盘且终身统计不丢；线上更新元数据和真实 LiteSnap 双向滚动均有独立只读/隔离入口 |
| [x] | LL-509 | LiteSnap 每工具独立粗细 | P1 | 完成 | 矩形、椭圆、直线、箭头、画笔、荧光笔、马赛克和模糊分别记忆 1–60 px；旧统一值无损迁移，文字/序号继续使用字号 |
| [x] | LL-510 | LiteSnap 标注编辑、文字与贴图缩放 | P1 | 完成 | 标注画完即可移动/缩放，文字支持连续输入、IME 防竞态和双击重编辑；F3 改为原生整体等比缩放并保留原分辨率导出 |
| [x] | LL-511 | 插件操作区紧凑化与 F3 拖动稳定性 | P1 | 完成 | JSON/CSV、密码、字符串、硬件、图片提示词和剪贴板操作区改为紧凑选择控件；F3 内容拖动锁定当前尺寸，Cron 模板按钮补齐暗色主题 |
| [x] | LL-512 | 大显存 GPU 检测修复 | P1 | 完成 | 显存改用 Windows 驱动 64 位数据并以 NVIDIA 驱动查询回退；旧 WMI 4 GB 截断值不再当作真实容量，界面展示数据来源与未直连显示器说明 |
| [x] | LL-513 | Hardware Inspector 准确识别与界面重构 | P1 | 完成 | 离线厂商中文名、显存分类、显示器集合、匿名分阶段诊断、六卡概览、内存汇总和按需报告预览；刷新失败保留旧数据 |

## 6. 下一步建议

1. v1.1.20 已完成 Hardware Inspector 厂商解析、显存分类、分阶段诊断和界面重构，并通过完整回归、Electron smoke 与 Windows 打包校验。
2. 发布后只读验证 `v1.1.19 -> v1.1.20` Latest Release、`latest.yml`、NSIS、blockmap、大小与发布说明，不下载安装包。
3. 已使用安装包无安装解包和共享隔离数据验证 `v1.1.19 -> v1.1.20` 设置、主题、历史与更新器兼容；真实 NSIS 原位升级继续只允许在一次性 Windows 环境运行。
4. 当前已无未完成的 P0/P1；下一批优先来自用户真实反馈，不自动扩展大型功能。
