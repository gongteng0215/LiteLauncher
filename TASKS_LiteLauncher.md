...
| [ ] | LL-209 | WebTools 19 工具迁移收尾 | P0 | 进行中 | 目录已建完，当前仅 6 个可见 |
| [ ] | LL-210 | 插件面板注册器收敛 | P0 | 进行中 | 仍有部分面板逻辑在 `renderer.ts` |
| [ ] | LL-216 | 插件可见性配置化 | P1 | 待办 | 现在在 `index.ts` 手工白名单 |
...
| [ ] | LL-505 | E2E 自动化（搜索+插件+设置） | P1 | 待办 | 未接入 |
| [ ] | LL-506 | WebTools 可见插件回归脚本 | P0 | 待办 | 需覆盖 6 个可见插件 |

## 6. 下阶段优先级建议

1. 推进 `LL-209`：按高频优先开放下一批隐藏 WebTools 插件。
2. 补 `LL-506` + `LL-505`：建立搜索 / 插件 / 设置页自动回归。
