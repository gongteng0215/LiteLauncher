...
1. v1.1.16 收口 LL-207 Cashflow 最近 12 局复盘与 LL-208 稳健、均衡、进取三种 AI 性格策略。
2. 发布门禁覆盖 Cashflow 源码回归、完整 Electron smoke、UTF-8 检查、Windows NSIS/ZIP 打包与发布资产校验。
3. 当前已无未完成的 P0/P1；下一批优先来自用户真实反馈，不自动扩展大型功能。
4. 后续 NSIS 原位升级只允许在明确的一次性 Windows 环境中运行；普通本机由脚本硬门禁拒绝。
5. 发布后重点观察旧单 AI 存档自动补齐、历史复盘切换和三种 AI 决策差异。
