...
- 同一目标命中多条结果时，应保证其中至少一条带完整标题、可执行 target 和可解析图标
...
如果插件存在默认示例、默认表达式或默认参数，面板打开后应自动完成首轮渲染，避免用户进入后看到空白结果区。

## 7. 交互规范

- `Enter`：执行当前插件主动作
- 多行输入框内 `Enter`：换行
- 多行输入框内 `Ctrl+Enter`：执行插件主动作
- `Esc`：返回搜索页
- 关键结果必须通过状态栏提示成功/失败
- 耗时动作必须提供加载态或处理中反馈
- 默认示例型插件应在进入面板后自动给出首屏结果
- 自动联动型插件应在输入变化后自动刷新结果，但仍要保留明确主动作供 `Enter` 触发
- 长结果区可滚动，关键操作提供按钮（如复制）

## 8. 原生交互规范

- 如果插件会触发系统文件选择框、保存对话框、下载动作或其它会导致窗口失焦的原生交互，必须显式接入“自动隐藏暂停”链路。
- 原生交互结束后要及时释放暂停状态，避免主窗口长期不自动隐藏。
- 典型场景：图片选择、二维码下载、图片下载。
...
3. 开放到默认可见前，至少完成：
   ...
   - 小屏布局回归
   - 基础回归脚本覆盖
4. 插件进入默认可见列表时，需同步更新默认可见插件迁移逻辑，保证老配置可平滑升级
