...
const baselineInstaller = path.resolve(
  process.argv[2] ||
    path.join(projectRoot, "artifacts", "release-baseline", "v1.1.17", "LiteLauncher-Setup-1.1.17.exe")
);
const candidateInstaller = path.resolve(
  process.argv[3] || path.join(projectRoot, "release", "LiteLauncher-Setup-1.1.18.exe")
);
const baselineVersion = process.argv[4] || "1.1.16";
const candidateVersion = process.argv[5] || "1.1.18";
